//
//  MessageTwoButtonsView.swift
//  SwiftMessages
//
//  Created by Hui Wang on 2021-09-22.
...
/*
 */
open class MessageTwoButtonsView: BaseView, Identifiable, AccessibleMessage {
    ...
    open var negativeButtonTapHandler: ((_ button: UIButton) -> Void)?
    
    @objc func negativeButtonTapped(_ button: UIButton) {
        negativeButtonTapHandler?(button)
    }
    
    open var positiveButtonTapHandler: ((_ button: UIButton) -> Void)?
    
    @objc func positiveButtonTapped(_ button: UIButton) {
        positiveButtonTapHandler?(button)
    }
    ...
    @IBOutlet open var negativeButton: UIButton? {
        didSet {
            if let old = oldValue {
                old.removeTarget(self, action: #selector(MessageTwoButtonsView.negativeButtonTapped(_:)), for: .touchUpInside)
            }
            if let button = negativeButton {
                button.addTarget(self, action: #selector(MessageTwoButtonsView.negativeButtonTapped(_:)), for: .touchUpInside)
            }
        }
    }
    
    @IBOutlet open var positiveButton: UIButton? {
        didSet {
            if let old = oldValue {
                old.removeTarget(self, action: #selector(MessageTwoButtonsView.positiveButtonTapped(_:)), for: .touchUpInside)
            }
            if let button = positiveButton {
                button.addTarget(self, action: #selector(MessageTwoButtonsView.positiveButtonTapped(_:)), for: .touchUpInside)
            }
        }
    }
    ...
    open var id: String {
        get {
            return customId ?? "MessageTwoButtonsView:title=\(String(describing: titleLabel?.text)), body=\(String(describing: bodyLabel?.text))"
        }
        set {
            customId = newValue
        }
...
/*
 ...
 `MessageTwoButtonsView` from the included nib files in a type-safe way. These nib
...
extension MessageTwoButtonsView {
    ...
    public enum Layout: String {
        ...
        /**
         ...
         container view.
         */
        case MessageTwoButtonsView = "MessageTwoButtonsView"
         ...
         */
        case centeredTwoButtonsView = "CenteredTwoButtonsView"
    }
    
    /**
     Loads the nib file associated with the given `Layout` and returns the first
     view found in the nib file with the matching type `T: MessageTwoButtonsView`.
     
     - Parameter layout: The `Layout` option to use.
     - Parameter filesOwner: An optional files owner.
     
     - Returns: An instance of generic view type `T: MessageTwoButtonsView`.
     */
    public static func viewFromNib<T: MessageTwoButtonsView>(layout: Layout, filesOwner: AnyObject = NSNull.init()) -> T {
    ...
    /**
     ...
     file with the matching type `T: MessageTwoButtonsView`.
     ...
     - Returns: An instance of generic view type `T: MessageTwoButtonsView`.
     */
    public static func viewFromNib<T: MessageTwoButtonsView>(layout: Layout, bundle: Bundle, filesOwner: AnyObject = NSNull.init()) -> T {
...
extension MessageTwoButtonsView {
    ...
    public func configureTheme(backgroundColor: UIColor, foregroundColor: UIColor, iconImage: UIImage? = nil, iconText: String? = nil) {
        ...
        negativeButton?.backgroundColor = foregroundColor
        negativeButton?.tintColor = backgroundColor
        negativeButton?.contentEdgeInsets = UIEdgeInsets(top: 7.0, left: 7.0, bottom: 7.0, right: 7.0)
        negativeButton?.layer.cornerRadius = 5.0
        positiveButton?.backgroundColor = foregroundColor
        positiveButton?.tintColor = backgroundColor
        positiveButton?.contentEdgeInsets = UIEdgeInsets(top: 7.0, left: 7.0, bottom: 7.0, right: 7.0)
        positiveButton?.layer.cornerRadius = 5.0
...
extension MessageTwoButtonsView {
     ...
     */
    public func configureContent(title: String?, body: String?, iconImage: UIImage?, iconText: String?, negativeButtonImage: UIImage?, negativeButtonTitle: String?, negativeButtonTapHandler: ((_ button: UIButton) -> Void)?, positiveButtonImage: UIImage?, positiveButtonTitle: String?, positiveButtonTapHandler: ((_ button: UIButton) -> Void)?) {

        titleLabel?.text = title
        bodyLabel?.text = body
        iconImageView?.image = iconImage
        iconLabel?.text = iconText
        negativeButton?.setImage(negativeButtonImage, for: .normal)
        negativeButton?.setTitle(negativeButtonTitle, for: .normal)
        self.negativeButtonTapHandler = negativeButtonTapHandler
        positiveButton?.setImage(positiveButtonImage, for: .normal)
        positiveButton?.setTitle(positiveButtonTitle, for: .normal)
        self.positiveButtonTapHandler = positiveButtonTapHandler
        iconImageView?.isHidden = iconImageView?.image == nil
        iconLabel?.isHidden = iconLabel?.text == nil
    }
}
